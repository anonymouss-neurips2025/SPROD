...
class OpenMax(BasePostprocessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loder_dict, model_name):
        if not self.setup_flag:
            # Fit the weibull distribution from training data.
            print('Fittting Weibull distribution...')
            _, mavs, dists = compute_train_score_and_mavs_and_dists(
                self.nc, id_loader_dict['train'], device='cuda', net=net, model_name=model_name)
    ...
    @torch.no_grad()
    def postprocess(self, net: nn.Module, data, use_features: bool, model_name: str):
        net.eval()
        if use_features:
            if 'resnet' in model_name:
                scores = net.model.fc(data)
            elif 'clip' in model_name:
                scores = net.proj(data)
            else:
                scores = net.model.head(data)
        else:
...
def compute_train_score_and_mavs_and_dists(train_class_num, trainloader,
    ...
    with torch.no_grad():
        for train_step in tqdm(range(1,
            ...
            target = batch[1].cuda()
            if 'resnet' in model_name:
                outputs = net.model.fc(data)
            elif 'clip' in model_name:
                outputs = net.proj(data)
            else:
                outputs = net.model.head(data)
